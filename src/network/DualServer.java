...
/**
 * Runs both a TCP and a UDP server. Creates two {@link java.lang.Thread
 * Threads}, one for each server.
 *
 * @author Joe Desmond
 */
public final class DualServer {
	...
	/**
	 * Starts the TCP and UDP servers; they will listen on their respective ports
	 * until a client connects or sends data.
	 */
	public void start() {
		tcpThread.start();
		udpThread.start();
	...
	/**
	 * Sends a message to the TCP Server. If the TCP Server has not been started, it
	 ...
	 *
	 * @param message
	 *            String to send through TCP Server
	 */
	...
	/**
	 * Returns the latest, new String received by the TCP server. If the latest
	 * message is not new, returns null.
	 *
	 * @return latest message received from TCP Server, unless message has been
	 *         checked before
	 */
	...
	/**
	 * Returns the latest, new String received by the UDP server. If the latest
	 * message is not new, returns null.
	 *
	 * @return latest message received from UDP Server, unless message has been
	 *         checked before
	 */
