...
/**
 * Sends and receives TCP packets in a connection with one other device. When
 * <code>run()</code> is called, this <code>TCPServer</code> begins listening on
 * the port specified in the constructor. When another device successfully
 * connects, communication can begin.
 *
 * <p>
...
public class TCPServer implements Runnable {
	...
	/**
	 * Creates a <code>TCPServer</code> that will send and receive data through the
	...
	/**
	 * Creates a new <code>TCPServer</code> with the specified port and
	 * <code>Communicator</code>. Returns a new <code>Thread</code> with this
	 * <code>TCPServer</code>, but does not start the <code>Thread</code>.
	 *
	 * @param port
	 *            port to create <code>TCPServer</code> with
	 * @param communicator
	 *            <code>Communicator</code> to create <code>TCPServer</code> with
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				System.out.println("TCP Server connected to " + socket.getInetAddress().getHostAddress()
